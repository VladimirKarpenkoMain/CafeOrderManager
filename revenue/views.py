...
class RevenueView(TitleMixin, ListView):
    ...
    def get_queryset(self):
        """
        Возвращает все записи о выручке, отсортированные по дате создания (по убыванию).
        """
        return Revenue.objects.all().order_by('-created_at')

    def get_context_data(self, **kwargs):
        """
        Формирует контекст для шаблона, добавляя информацию о выручке за сегодняшний день
        и общий список записей о выручке.
        """
...
class CreateRevenueView(View):
    ...
    def post(self, request, *args, **kwargs):
        """
        Обновляет данные о выручке. Выводит сообщение об успехе или ошибке.
        """
